...
export default class Database {
  ...
  // New method to retrieve payments by wallet_address
  async getPaymentsByWallet(wallet: string) {
    const res = await this.client.query('SELECT * FROM payments WHERE wallet_address = $1 ORDER BY timestamp DESC', [wallet]);
    return res.rows;
  }
}
